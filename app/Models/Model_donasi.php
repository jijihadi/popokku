...
class Model_donasi extends Model
{
    ...
    {
        $idkurir = $this->db->table('tb_kurir')->where('id_user', $id)->get()->getresult();
        $idkurir = $idkurir['0']->id_kurir;
        // return $idkurir;
        $builder = $this->db->table('tb_donasi a')
            ->select('a.*')
            ->orderby('tanggal_pengajuan', 'desc')
            ->where('a.id_kurir', $idkurir)->get();
        return $builder;
    }

    public function get_donasi_notyet_bykurir($id)
    {
        $idkurir = $this->db->table('tb_kurir')->where('id_user', $id)->get()->getresult();
        $idkurir = $idkurir['0']->id_kurir;
        // return $idkurir;
        $builder = $this->db->table('tb_donasi a')
            ->select('a.*')
            ->orderby('tanggal_pengajuan', 'desc')
            ->where('status_donasi not like 2')
            ->where('a.id_kurir', $idkurir)->get();
        return $builder;
    }
