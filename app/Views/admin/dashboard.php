            ...
            <table class="table table-stripped table-striped dtr-inline">
                ...
                <tbody>
                    ...
                    <tr>
                        <td><b><?=$i;?></b></td>
                        <td><?=tgl_indo(date('Y-m-d H:i', strtotime($row['tanggal_pengajuan'])));?></td>
                        <td><?= ucwords(shownamauser($row['id_user']));?></td>
                        <td><?=$row['jumlah_donasi'];?></td>
                        <td><?=$row['alamat_jemput'];?></td>
                        <td><?=tgl_indo(date('Y-m-d H:i', strtotime($row['tanggal_diambil'])));?></td>
                        <td class="row">
                        <button class="btn btn-success m-2" onclick=" window.open('https://www.google.com/maps/search/?api=1&query=<?=$row['lati'].",".$row['longi'];?>','_blank')"> Arahkan Saya</button>
                        <a href="https://wa.me/<?=shownowa($row['id_user'])?>?text=Hay kak, saya kurir popoku, saya mau kesana untuk mengambil limbah popok, apakah kakak bersedia?" target="none" class="btn btn-primary m-2" > Hubungi Donatur</a>
                        </td>
                    </tr>
